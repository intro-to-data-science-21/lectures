...
- The **user space** provides an interface for users, who can run programs/applications on the machine. Hardware access of programs (e.g., memory usage) is managed by the kernel. Programs in user space are essentially in sandboxes, where there is a limit to how much damage they can do.
]
...
.footnote[<sup>1</sup>That's what I use, which is why the shell on the following slides might look a bit different. Also, some commands/programs might or might not be available (and need to be installed).]
]
...
6. **Become a marketable data scientist.** Modern programming is often polyglot. The shell provides a common environment and interface for tooling. Modern solutions are often built to run in containers on Linux. In this environments shell knowledge has become very valuable. In short, [the shell is having a renaissance](https://effective-shell.com/docs/part-1-transitioning-to-the-shell/the-renaissance-of-the-shell/) in the age of data science.
...
Let's open up our shell.<sup>1</sup>
...
.footnote[
<sup>1</sup>Windows users: See [here](#wsl-rstudio).
]
...
<br>
<div align="center">
<img src="pics/beatles-help.jpeg" height=400/>
</div>
]
...
.pull-left[
- We're all so used to a graphical user interface that switching to the shell can take some getting used to. 
...
For example, say we want to change the file type (i.e. extension) of a set of files in the `examples/meals` directory: 
...
```
```{bash find3}
find . -size +1000k ## find files larger than 1000 KB
```
