...
- There are various categories of selectors. In addition to generic element selector (which are selected just based on the element name, such as `<p>`), we often care about:
...
.footnote[
<sup>1</sup> There is more in `rvest`. Have a glimpse at the [overview at tidyverse.org](https://rvest.tidyverse.org/ and at this excellent (unofficial) [cheat sheet](https://github.com/yusuzech/r-web-scraping-cheat-sheet).
]
...
- The article provides two types of lists - one by period and one sorted alphabetically. We want the alphabetial list.
- The information we are actually interested in - names - are stored in unordered list elements.
]
...
<img src="pics/wiki-philosophers-4.png" height=220>
</div>
]
...
Having learned about a semi-automated approach to generating XPath expressions, you might ask: Why bother with learning XPath at all? Well...
...
- (Note that we're actually using an old version of the page (dating back to May 1, 2018, and accessible under  https://en.wikipedia.org/w/index.php?title=List_of_human_spaceflights&oldid=778165808. Wikipedia pages change, but this old revision won't.))
...
Model fitting and simulation. Now that we know how to retrieve data, let's learn how to learn from them.
